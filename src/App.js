...
function App() {
  ...
  function handleChange(e) {
    ...
    for (let i = typedText.length; i < question[randomIndex].length; i++) {
      updatedText.push(<span>{question[randomIndex][i]}</span>);
    }
    setPara({
      typed: updatedText
    })
  ...
  return (
    ...
    {para && (
      <>
        ...
        <div>{para.typed}</div>
        <div style={{ position: 'absolute', top: '10px', right: '10px', fontSize: '20px' }}>Timer: {timer}s</div>
        <form>
          <input name="type" placeholder="Type" onChange={handleChange}autoFocus />
        </form>
      </>
    )}
    {finished &&(
      <h1 style={{ textAlign: 'center' }}>You're done. Your typing speed is {Math.round((correctWords/ 30) * 60)} WPM.</h1>
    )}
  </div>
);
}
